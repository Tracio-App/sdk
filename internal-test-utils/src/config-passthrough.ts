...
export function checkConfigPassthrough(received: unknown): {
  ...
  if (received === null || typeof received !== "object") {
    return {
      ok: false,
      problems: [`Tracio.init получил ${String(received)}, а не объект конфига`],
    };
  }
  ...
  for (const [key, expected] of Object.entries(FULL_CONFIG)) {
    if (!(key in got)) {
      problems.push(`поле «${key}» не дошло до ядра — обёртка его теряет`);
      continue;
    }
    if (got[key] !== expected) {
      problems.push(
        `поле «${key}»: ожидалось ${JSON.stringify(expected)}, получено ${JSON.stringify(got[key])}`,
      );
    }
  }
  ...
  for (const key of Object.keys(got)) {
    if (!(key in FULL_CONFIG)) {
      problems.push(`обёртка добавила лишнее поле «${key}» — ядро его не ожидает`);
    }
  }
...
export function expectConfigPassthrough(received: unknown): void {
  ...
  if (!ok) {
    throw new Error("Обёртка передала конфиг в ядро не полностью:\n  - " + problems.join("\n  - "));
  }
}
