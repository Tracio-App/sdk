...
export function injectScript(
  ...
  if (existing) {
    // The bootstrap reads `data-tracio-fields` off document.currentScript when
    // /s.js EXECUTES (back/edge/internal/serve/script.go) and never re-reads it,
    // so setting the attribute now would be a no-op on an already-loaded tag —
    // or would smuggle our fields into someone else's integration. Warn instead.
    if (fields && Object.keys(fields).length > 0) {
      console.warn(
        "[tracio] a script tag for this key already exists; custom fields were NOT sent. " +
          "Put them on that tag's data-tracio-fields, or remove the tag before init.",
      );
    }
    return existing;
  }
  ...
  // Custom fields ride on the SAME attribute a hand-written tag uses: the edge
  // bootstrap reads `data-tracio-fields` off `document.currentScript`, and for
  // an SDK-driven load that is this very element. Set BEFORE appending — the
  // bootstrap runs on load and never re-reads the attribute. An empty map is
  // not sent: the attribute's absence and `{}` mean the same thing to the edge,
  // and omitting it keeps the tag identical to a non-fields integration.
  if (fields && Object.keys(fields).length > 0) {
    script.setAttribute("data-tracio-fields", JSON.stringify(fields));
  }
