...
import type { TracioAdvice, TracioBotInfo, TracioInlineGuidance, TracioResult } from "./types.js";
...
interface RawTracioResponse {
  ...
  markers?: string[];
  /**
   * Id of this identification request — the value the signed verdict below is
   * bound to. Source: `resp["request_id"] = requestID` in `attachInlineVerdict`
   * (`back/edge/internal/serve/collect.go`).
   */
  request_id?: string;
  /**
   * Verdict computed before the response was written. Only a workspace with
   * rules or lists gets it, so its absence is the normal case, not a failure.
   */
  inline_guidance?: {
    version?: number;
    overall?: string;
  ...
  /** `true` when asynchronous axes were left out of `inline_guidance`. */
  inline_partial?: boolean;
  /** `inline_guidance` signed by the edge — `tracio_vt1.<payload>.<sig>`. */
  inline_token?: string;
}
...
/**
 * The four advices the verdict vocabulary allows (`back/shared/guidance`).
 * Anything else did not come from an edge we know.
 */
const ADVICE_VALUES: readonly string[] = ["allow", "challenge", "review", "deny"];

function toAdvice(raw: unknown): TracioAdvice | undefined {
  return typeof raw === "string" && ADVICE_VALUES.includes(raw) ? (raw as TracioAdvice) : undefined;
}

/**
 * Map the edge's `inline_guidance` into {@link TracioInlineGuidance}.
 *
 * All or nothing: the six fields are written by one builder on the edge, so a
 * response missing any of them — or carrying an advice outside the vocabulary
 * — is not a partially valid verdict, it is not our verdict. Handing the
 * caller a half-filled object typed as complete would be the worse failure:
 * `if (guidance.payment === "deny")` would read `undefined` as "allow".
 *
 * A dropped object costs nothing that matters: `inline_token` still carries
 * the same verdict, and that is the copy a backend is supposed to decide on.
 */
function normalizeInlineGuidance(
  raw: RawTracioResponse["inline_guidance"],
): TracioInlineGuidance | undefined {
  if (!raw || typeof raw !== "object") return undefined;
  const overall = toAdvice(raw.overall);
  const payment = toAdvice(raw.payment);
  const registration = toAdvice(raw.registration);
  const login = toAdvice(raw.login);
  const affiliate = toAdvice(raw.affiliate);
  if (typeof raw.version !== "number") return undefined;
  if (!overall || !payment || !registration || !login || !affiliate) return undefined;
  const basis = Array.isArray(raw.basis)
    ? raw.basis.filter((item): item is string => typeof item === "string")
    : undefined;
  return {
    version: raw.version,
    overall,
...
export async function loadAndFetch(args: LoadAndFetchArgs): Promise<TracioResult> {
  ...
  const inlineGuidance = normalizeInlineGuidance(raw.inline_guidance);
  // The flag qualifies the verdict, so it travels with it and never alone —
  // on its own it would describe nothing. When the verdict is there and the
  // flag is not, we say "incomplete": the only unsafe direction is claiming a
  // completeness this channel cannot deliver (the asynchronous axes land after
  // the response), and a caller reading `!inlinePartial` would take exactly
  // that reading.
  const inlinePartial = inlineGuidance
    ? typeof raw.inline_partial === "boolean"
      ? raw.inline_partial
      : true
    : undefined;
  ...
  return {
    ...
    ...(typeof raw.request_id === "string" && raw.request_id ? { requestId: raw.request_id } : {}),
    ...(inlineGuidance ? { inlineGuidance } : {}),
    ...(inlinePartial !== undefined ? { inlinePartial } : {}),
    // The token rides independently of the object above: it is the copy a
    // backend verifies and decides on, and our failure to parse the plain
    // object is no reason to withhold the signed one.
    ...(typeof raw.inline_token === "string" && raw.inline_token
      ? { inlineToken: raw.inline_token }
      : {}),
  };
}
