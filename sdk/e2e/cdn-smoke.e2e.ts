...
// tag) and the data-tracio-fields attribute were never exercised in a browser.
// A regression there is invisible to happy-dom too, where the script never runs.
test.describe("real URL building (no scriptUrl)", () => {
  test("puts key, account linking and tag in the query, and fields on the tag", async ({
    page,
  }) => {
    await page.goto("/e2e/fixtures/iife.html");
    await page.waitForFunction(
    ...
    const out = await page.evaluate(async () => {
      ...
      return {
        ...
        fields: (window as Record<string, any>)["__tracioTagFields"],
      };
    });
    ...
    expect(JSON.parse(out.fields as string)).toEqual({ plan: "pro", order: "A-1029" });
  });
});
