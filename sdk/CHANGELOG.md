...
- 9c97e35: Re-release of the `fields` change: 0.2.2 never reached npm.

  The publish workflow gates on `prettier --check`, and the `fields` row rewritten
  in 0.2.2 was longer than the column width the rest of the options table was
  padded to — so the table no longer matched Prettier's output and the release
  stopped before publishing. Nothing about the package's behaviour differs from
  what 0.2.2 described; that version number is simply skipped.

## 0.2.2

### Patch Changes

- 7e31584: `fields` no longer touch the page markup: the SDK hands them to the edge
  runtime as an argument of its `get()` call instead of the
  `data-tracio-fields` attribute.

  The attribute sat on the injected `<script>` for the life of the page, and
  the edge bootstrap copied it into a `window` global on top — so every other
  script on the page (analytics tags, A/B tools, session replay) could read
  whatever the integrator put there, and the README's own example was an email.
  Now the map lives in the runtime's memory only: it is never written to the
  DOM, the script URL or `window`.

  No code change on your side — `Tracio.init({ fields })` and the framework
  wrappers keep the same signature. The edge runtime that accepts the argument
  is served by Tracio; an older runtime ignores it and the fields are not sent.
  Two behaviours changed alongside:
  - A page that already carries a hand-written tag for the same key: the fields
    are now handed to that tag's runtime instead of being dropped, and are
    recorded as long as it is still collecting (first wave at load, late wave
    on the first hide). The warning stays, reworded, because a runtime that has
    finished has nothing left to attach them to. The tag's own attribute is
    never touched.
  - `destroy()` now also unsets the `window.Tracio` its bundle installed. The
    documented SPA recipe — `destroy()` then `init({ fields })` — used to pick up
    the old bundle's global synchronously, so the new fields went to a runtime
    whose visit was over; now the re-`init` waits for the runtime of the tag it
    injects.
  - The README no longer claims a non-string value throws `invalid_config`; the
    SDK warns and sends it, as it has since 0.2.0.

  Personal data (an email, a phone number) still belongs on the server side:
  `POST /v1/accounts/metadata` keyed by your `linkedId`. See "Custom fields" in
  the README.
...
- 99ef33b: Publishing moved to npm trusted publishing (OIDC); no package content changes.
...
- 6c0b66c: fix: `getResult()` returns the visitor id the rest of the platform uses, and
  `@tracio/react` stops dropping `region`, `linkedId` and `tag`

  **Two defects, both silent.** Neither threw, logged, nor failed a build — the
  integration looked wired up and simply did not do what it promised.

  `getResult().visitorId` preferred `canonical_uid` over `visitorId`. Those are
  two different identifiers: `visitorId` is what the dashboard shows and what the
  Data API accepts; `canonical_uid` is a client-side persistence UUID. Every
  lookup by the returned value answered 404, and because both are opaque strings
  there was nothing to suggest which one was wrong.

  `@tracio/react` rebuilt the config field by field — deliberately, so the effect
  depends on primitives and an inline `config={{…}}` object does not re-init the
  SDK on every render — but the enumeration was never extended when the core
  gained new fields. `linkedId` was added to the core two weeks after the wrapper
  was written and was never referenced in it; `tag` and `region` fell into the
  same hole later. Passing `linkedId` through the React wrapper therefore did
  nothing: account linking, Connections and fraud-ring grouping stayed empty while
  the integration appeared correct.

  **Read this before upgrading.** Although this ships as a patch (the packages are
  pre-1.0), the observable behaviour changes in two ways. `getResult()` now
  returns a different string than 0.1.3 did for the same visitor — ids you
  persisted from 0.1.x do not match what the dashboard and Data API know, so treat
  them as unusable for lookups rather than assuming continuity. React users who
  already pass `linkedId` will see account linking begin to work, which changes
  what the edge records for those visitors.

  **Guard against a recurrence.** `TRACIO_CONFIG_KEYS` is now exported as the
  runtime reflection of the `TracioConfig` surface, locked to the interface in
  both directions by a type-level assertion, along with
  `AssertWrapperCoversConfig<Props>` for wrappers that expose config as flat
  props. Adding a field to `TracioConfig` now fails the build until that field is
  listed, present in the shared test fixture, accepted by each wrapper, and
  observed reaching `Tracio.init` by a pass-through test in every framework
  package. The type guard proves a wrapper _accepts_ a field; the runtime test
  proves it _forwards_ one — a wrapper can declare a prop and still forget to put
  it into the object it hands to the core, which is exactly what happened here and
