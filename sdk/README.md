...
```ts
type Advice = "allow" | "challenge" | "review" | "deny";

type TracioResult = {
  visitorId: string;
  bot: { detected: boolean; confidence: number /* 0–100 */; reasons?: string[] };

  // Verdict channel — present only for a workspace that has rules or lists.
  requestId?: string;
  inlineGuidance?: {
    version: number;
    overall: Advice; // strictest of the four below — a summary, not a decision
    payment: Advice;
    registration: Advice;
    login: Advice;
    affiliate: Advice;
...
### Verdict in the response (`inlineGuidance`)

Your rules and lists are applied **before** the identification response is written, so
the advice arrives with it — no second request, no API quota. Four fields for the four
places a decision is usually made, plus `overall`, which is only the strictest of them:
a visitor farm can ban the affiliate conversion while the payment is fine, so decide on
the scenario that matches your integration.

The four `inline*` fields are optional, and each absence means something specific:

- **no `inlineGuidance`** — this workspace has no rules and no lists, so there is nothing
  to compute an advice from. The channel exists for workspaces that have at least one.
- **no `inlineToken`** — the edge node answered without its signing key. You get the
  verdict, unsigned, rather than nothing at all.
- **`inlinePartial: true`** — axes were left out. Today that is always the case: account
  takeover, person, reputation and behaviour land after this response by construction, so
  the webhook can advise more strictly on the same visit. That is the declared shape of
  this channel, not a discrepancy.

**Decide on `inlineToken`, not on the object.** The object lives in the browser, and a bot
we detected can rewrite it before your page submits the form. `inlineToken` carries the
same verdict signed by the edge (Ed25519, the same `t=…,kid=…,v1=…` format and the same
verification path as our webhooks, with its own `kid` published at
`/.well-known/webhook-keys`). Send it to your backend, verify it there, and compare the
`requestId` inside it with the request you are handling — an unchecked token issued for
another visit passes just as well as your own. Use `inlineGuidance` for what the browser
itself does: render a challenge, disable a button.

```ts
const result = await tracio.getResult();
if (result.inlineGuidance?.payment !== "allow") {
  showExtraCheck();
}
await fetch("/api/checkout", {
  method: "POST",
  headers: { "content-type": "application/json" },
  // The backend verifies the token and decides; the line above is UI only.
  body: JSON.stringify({ ...order, tracioToken: result.inlineToken }),
});
```
