...
| `fields`       | `Record<string, string>`         | —                          | Custom fields attached to every event of this page load (e.g. `{ email, plan }`). Sent as `data-tracio-fields` on the injected script. Values must be strings; see below. |
...
reference, a plan, the signed-in user's email:

```ts
const tracio = Tracio.init({
  publicKey: "tracio_pk_…",
  fields: { email: user.email, plan: "pro" },
});
```

The SDK puts the map on the injected `<script>` as `data-tracio-fields` — the
same channel a hand-written tag uses — so the edge treats both identically.
Values must be **strings**; a number or boolean is an `invalid_config` error
(a hand-written tag would coerce it silently, the typed SDK does not).
Limits are enforced server-side and apply per field, not per visit: 50 fields,
key ≤ 40 bytes matching `[A-Za-z0-9_.:-]`, value ≤ 500 bytes, no ASCII control
characters. A field over the limit is dropped and counted in the dashboard's
rejected-fields counter; the visit itself is always recorded. The page gets no
synchronous feedback — check the dashboard or the Data API (`fields` on a
session). Values must be known when the page renders; for values that arrive
later (order total, payment outcome) use `POST /v1/events/fields` in the Data
API. Values are stored in clear only inside your own workspace and never enter
the cross-customer graph. In SPAs the same rule as for `linkedId` applies:
`destroy()` and re-`init` to change fields — a live instance ignores new config.
