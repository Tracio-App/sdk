...
describe("loader", () => {
  ...
  describe("loadAndFetch", () => {
    it("e2e: инжект → wait → get → нормализация в TracioResult", async () => {
        ...
        load: () => ({
          get: () =>
            Promise.resolve({
              visitor_id: "vid-1",
              request_id: "req-1",
              bot: { detected: true, confidence: 0.9, reasons: ["headless"] },
            }),
        }),
      };
      ...
      expect(result.bot.confidence).toBe(0.9);
        ...
        load: () => ({
          get: () =>
            Promise.resolve({
              visitor_id: "x",
              request_id: "y",
              bot: { detected: false, confidence: 0 },
            }),
        }),
      };
      await loadAndFetch({
    ...
    it("частичный/отсутствующий bot нормализуется в дефолты (не TypeError)", async () => {
      ...
      (window as any).Tracio = {
        load: () => ({
          // bot отсутствует целиком, request_id отсутствует
          get: () => Promise.resolve({ visitor_id: "vid-partial" }),
        }),
      };
      const result = await loadAndFetch({
        endpoint: "https://edge.tracio.test",
        ...
        load: () => ({
          get: () =>
            Promise.resolve({
              ...
              request_id: "req-p2",
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              bot: { detected: true, reasons: "not-an-array" as any },
            }),
        }),
      };
      const result = await loadAndFetch({
