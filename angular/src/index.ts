...
export class TracioService implements OnDestroy {
  ...
  private instance: TracioInstance | null = null;
  ...
  /**
   * Release the SDK instance when this injector is destroyed.
   *
   * Provide `provideTracio()` in `ApplicationConfig.providers`, where the
   * injector lives as long as the app. Placing it in a route's or component's
   * providers ties the instance to that scope: leaving the route destroys it
   * and removes the injected `<script>`, and coming back injects a new one —
   * a second recorded visit within the same page load.
   */
  ngOnDestroy(): void {
